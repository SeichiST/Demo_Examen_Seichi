	...
	void listar() {
		...
		try {
			...
			for (OrdenSoporte ordenSoporte : lstOrdenes) {
				...
				imprimir("Tecnico...............: " + tecnico.getNombre() + " especialista en " + tecnico.getEspecilidadDescripcion());
	...
	void consultar() {
		...
		try {
			...
			if (ordenSoporte == null) {
				mensajeAdvertencia("Orden de soporte no encontrado");
				return;
			}
			...
			txtFechaRegistro.setText(ordenSoporte.getFechaRegistro().toString());
		} finally {
			manager.close();
		}
	}
